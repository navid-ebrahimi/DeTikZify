# -*- coding: utf-8 -*-
"""final_generating_images

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/#fileId=https%3A//storage.googleapis.com/kaggle-colab-exported-notebooks/final-generating-images-4d8d8850-3df3-48c5-914c-563026446e5b.ipynb%3FX-Goog-Algorithm%3DGOOG4-RSA-SHA256%26X-Goog-Credential%3Dgcp-kaggle-com%2540kaggle-161607.iam.gserviceaccount.com/20241108/auto/storage/goog4_request%26X-Goog-Date%3D20241108T122910Z%26X-Goog-Expires%3D259200%26X-Goog-SignedHeaders%3Dhost%26X-Goog-Signature%
"""
...
# !perl ./install-tl-20241108/install-tl --no-interaction
...
def add_documentclass_if_missing(tikz_code):
    ...
    if r"\documentclass" not in tikz_code:
        ...
        # If documentclass exists, return the code as is
        return tikz_code
...
def tikz2png_buffered(tikz_code, output_figure_name, process_id, io_buffer):
...
def save_metadata_to_parquet_limited(metadata, output_dir, base_filename="tikz_sample", max_file_size=50000 * 1024 * 1024):
    """
    Saves metadata to multiple Parquet files, each with a max size limit.
    ...
    for entry in metadata:
        ...
        # Create a DataFrame to calculate the size of the current batch
        df = pd.DataFrame(current_data)
        table = pa.Table.from_pandas(df)
        estimated_size = table.nbytes

        if current_file_size + estimated_size >= max_file_size:
            # Write the current batch to a Parquet file and reset
            file_path = os.path.join(output_dir, f"{base_filename}_{file_index}.parquet")
            pq.write_table(table, file_path)
            print(f"Saved {file_path} with approx size {current_file_size / (1024 * 1024):.2f} MB")
            ...
            # Accumulate size if below max limit
            current_file_size += estimated_size

    # Write any remaining data in the last file
    if current_data:
        file_path = os.path.join(output_dir, f"{base_filename}_{file_index}.parquet")
        table = pa.Table.from_pandas(pd.DataFrame(current_data))
        pq.write_table(table, file_path)
        print(f"Saved {file_path} with approx size {current_file_size / (1024 * 1024):.2f} MB")
...
def main_parallel(max_samples=400):
    ...
    for item in combined_metadata:
        try:
            original_image_path = os.path.join(output_dir, f"{item['original_image']}.png")
            original_image_binary = load_image_as_binary(original_image_path)

            sample_images_binary = []
            for sample_img in item['sample_images']:
                sample_img_path = os.path.join(output_dir, f"{sample_img}.png")
                sample_images_binary.append(load_image_as_binary(sample_img_path))
            ...
            data_for_parquet.append({
                ...
                "sample_images": sample_images_binary
            })
        except Exception as e:
            print(f"Error loading image: {e}")

    # Save to Parquet with file size limit
    save_metadata_to_parquet_limited(data_for_parquet, output_dir="parquet_output", max_file_size=50000 * 1024 * 1024)
